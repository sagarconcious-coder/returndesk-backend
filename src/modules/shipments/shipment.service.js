...
import { resolvePickupAddress } from "./shipment-address.helper.js";
import {
  getRepairByRmaId,
  createRepair,
...
import { createOrder as createShiprocketOrder } from "../../common/services/shiprocket.service.js";
...
// B) createShipment(rma_id, direction)
...
export const createShipment = async (rma_id, direction) => {
  ...
  let shipment;
  try {
    shipment = await shipmentRepo.createShipment(rma_id, direction);
  } catch (error) {
    if (error.code === "23505") {
...
const createOutboundOrder = async (rma, shipment) => {
  ...
  const order = await createShiprocketOrder({
    pickup_location:
      process.env.SHIPROCKET_WAREHOUSE_PICKUP_LOCATION || "warehouse",
